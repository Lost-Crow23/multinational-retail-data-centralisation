...
class DataCleaning:
    def __init__(self):
        ...
        def remove_duplicate_rows(self, df, column):
        ...
        def clean_user_data(self, user_data):
            ...
            # clean_user_data = self.country_code(clean_user_data)
        ...
        def drop_rows_with_null_values(self, df):
            ...
            df = df.duplicates(subset = ['phone_number', 'email_address']) # fix this
            return df
        ...
        def valid_email(self, df):
            df['email_address'] = df['email_address'].str.match[r'^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,4}$'] # rematch?
            return df
        ...
        def valid_phone_number(self, df):
            df['phone_number'] = df.apply(lambda row: re.sub(fr'^\+44|^\+49|^{row["country_code"]}','', row['phone number']), axis = 1) # fix this
            return df
        ...
        def clean_card_data(self, card_data):
            ...
            clean_card_data = self.remove_duplicate_rows(clean_card_data, column = ['card_number'])
        ...
        def clean_store_data(self, stores_data):
            ...
            cleaned_store_data = self.drop_store_null_values_(cleaned_store_data)
            ...
            cleaned_store_data = self.drop_invalid_store_typeC(cleaned_store_data)
        ...
        def convert_product_weights(self, weight):
            ...
            def convert(weight):
                if "kg" in weight :
                    new_wieght = float(weight.replace("kg", ""))
                elif "g" in weight:
...
if __name__ == "__main__":
    ...
    # store details 
    num_of_stores_endpoint = headers.json('my_endpoint')
    ...
    db_connector.upload_to_db('dim_store_details')
    db_connector.upload_to_db('dim_products')
    db_connector.upload_to_db('orders_table')  
    db_connector.upload_to_db('dim_date_times')
